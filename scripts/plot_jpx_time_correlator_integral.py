...
parsed from that name. With several directories, the running integrals of
...
def main() -> None:
    ...
    if args.output is AUTO_OUTPUT:
        stem = args.input_dirs[0].name.replace("obs-", "obs-integral-")
    ...
    ylabel = r"$\int_0^T \mathrm{d}t\, \mathrm{Re}\,\frac{1}{T'}\sum_{T'} \langle j_{x}^{*}(T'+t,\mathbf{k}) j_{x}(T',\mathbf{k})\rangle$"
    if len(set(modes)) == 1:
        ylabel += (r",  $\mathbf{k}=$"
