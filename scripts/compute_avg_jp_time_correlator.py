...
a directory named like the run but with "sim" replaced by "avg-jp-time-corr", the seed
stripped and the mode index appended (e.g. avg-jp-time-corr-Nx8Ny8Nz8dt0.1eta0.1Lam1.25nk7
for --nk 7). A trailing "x" suffix on the run name (written by sim for
...
def parse_run_name(run_dir: Path) -> dict:
    ...
    return {"nx": int(m["nx"]), "ny": int(m["ny"]), "nz": int(m["nz"]),
            ...
            "out_base": re.sub(r"^sim", "avg-jp-time-corr",
