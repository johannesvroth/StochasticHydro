#!/usr/bin/env python3
"""Compute the time correlator sum_T jpx(t)*jpx(t+T), averaged over all modes and t,
for a fixed mode magnitude nk from the raw jpx_re.dat/jpx_im.dat, jpy_re.dat/jpy_im.dat and jpz_re.dat/jpz_im.dat
...
a directory named like the run but with "sim" replaced by "avg-jp-time-corr" and the seed
stripped (e.g. avg-jp-time-corr-Nx8Ny8Nz8dt0.1eta0.1Lam1.25). A trailing "x" suffix on the
run name (written by sim for --no-ideal-step runs) is kept, so such runs go
to avg-jp-time-corr-...x.
...
def parse_run_name(run_dir: Path) -> dict:
    ...
    return {"nx": int(m["nx"]), "ny": int(m["ny"]), "nz": int(m["nz"]),
            "dt": float(m["dt"]),
            "out_dir": Path(re.sub(r"^sim", "avg-jp-time-corr",
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=None,
                         help="Directory to write the computed correlators into "
                              "(default: run dir name with 'sim' replaced by 'avg-jp-time-corr' "
    ...
    for run_dir in args.run_dirs:
        ...
        output_dir = args.output_dir if args.output_dir is not None else params["out_dir"]
